...
class BasicCritic(nn.Module):
    """
    NETD of WGAN SKIPGANOMALY, numerical value as output
    """
    def __init__(self, opt):
        ...
        for t in range(n_extra_layers):
            feat.add_module('extra-layers-{0}-{1}-conv'.format(t, cndf),
                            nn.Conv2d(cndf, cndf, 3, 1, 1, bias=False))
            feat.add_module('extra-layers-{0}-{1}-batchnorm'.format(t, cndf), #instanceNorm instead of BatchNorm
...
def define_C(opt, norm='batch', use_sigmoid=False, init_type='normal'): #define critic
    netD = None
    norm_layer = get_norm_layer(norm_type=norm)
    netD = BasicCritic(opt)
    return init_net(netD, init_type, opt.gpu_ids)
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        if outermost:
            upconv = nn.ConvTranspose2d(inner_nc*2, outer_nc, #inner_nc, outer_nc for deleting skip-connections
    ...
    def forward(self, x):
        if self.outermost: #True for deleting skip-connections
            return self.model(x)
        else:
